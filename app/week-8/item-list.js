...
export default function ItemList( {items, onItemSelect} ){
    ...
    return (
        <div>
            ...
            <div>
                {sortBy != "group category" ? itemsArray.map( (item) => <Item key={item.id} onItemSelect={onItemSelect} itemObj={item}/> ) : 
                ...
                itemsArray.map( (group) => (
                    <div key={group[0].category}>
                        ...
                        {group.map( (item) => <Item key={item.id} onItemSelect={onItemSelect} itemObj={item}/> )}
                    </div>
                ))}
            </div>
        </div>
